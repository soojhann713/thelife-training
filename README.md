...
**📝 과제 완주 현황** 카드의 **📄 취합문서 내보내기** 버튼에서 교회 양식(한글 `.hwpx`)에 채워
바로 내려받습니다. 양식은 저장소에 커밋된 **빈 양식을 그대로 써서** 서식을 한 글자도 고치지 않습니다.

| 양식 | 무엇이 나오나 | 파일명 예 |
| --- | --- | --- |
| **과제 취합문서** | 고른 과제 하나에 대한 반 전체 제출물. **멤버 1명 = 페이지 1장** | `11기_생활간증_20260503.hwpx` |
| **출석·과제현황표** | 주차별 체크리스트 한 장(1·2학기 표 2개) | `11기_출석과제현황_20260614.hwpx` |

- 과제는 **큰 분류(생활간증 / 독서 / 설교간증 …)를 고른 뒤** 그 안에서 고릅니다.
  목록은 **제출일 오름차순**입니다.
- 내려받기 전에 **미리보기**로 누가 제출했는지(또는 무엇이 채워지는지) 먼저 확인합니다.
  본문이 비어 있는 제출이 있으면 `backfill-content` 워크플로우를 먼저 돌리라고 알려줍니다.
- 현황표에서 **강의 출석(`출`) 칸은 비워서** 내보냅니다 — 수집할 방법이 없어 한글에서 직접 체크합니다.
  금(금요예배)·주(주일예배)는 **설교간증(예배은혜나눔) 제출**로 자동 표시됩니다.
- 현황표의 한 줄은 **그 강의일에 내준 과제**입니다(= 마감일 직전 강의일 줄).
  이 커리큘럼은 `마감 = 다음 강의일` 이라 1주차 과제는 3/15 에 걷지만 **3/8 줄**에 체크됩니다 —
  그래야 양식의 주차 번호와 커리큘럼 주차가 맞습니다(22주 = 10/18, 26주 = 11/15).
- 현황표는 양식이 가진 **주차 줄과 멤버 열(13명)** 을 그대로 씁니다. 멤버가 더 많으면 앞 13명만
  들어가고 미리보기가 알려줍니다. 양식이 취소선으로 "그 주엔 없음"이라 적어 둔 칸은 건드리지 않습니다.
- 채우는 기준·주의점은 [`PLAN-HWPX.md`](./PLAN-HWPX.md) §5·§7 에 정리돼 있습니다.

### 양식 도구 (`scripts/tools/`)

```bash
# 어느 과제가 어느 줄에 들어가는지 목록으로 확인 (내보내기와 같은 함수를 씁니다)
node scripts/tools/status-week-list.mjs > 목록.md

# 다음 기수 양식을 받았을 때: 이름·체크 기록을 지운 빈 양식 만들기
node scripts/tools/make-status-template.mjs <교회에서받은.hwpx>

# 양식의 어긋난 셀 병합 바로잡기 (모든 주차를 같은 6칸으로 — 멱등)
node scripts/tools/fix-status-form.mjs
```

`make-status-template` 출력 마지막 줄의 **‘값 칸에 남은 글자’와 ‘채우지 않는 행’** 을 확인하고,
‘채우지 않는 행’이 있으면 `fix-status-form` 을 돌린 뒤 커밋하세요.
