...
scripts/                          GitHub Actions 작업 (Node)
  ...
  audit-sermons.js                설교간증 자동 매칭 점검(출력 전용)
  test/                           단위 테스트 (큐티 파서 · 설교간증 매칭)
.github/workflows/
...
- **설교간증(예배은혜나눔)** 은 제목이 매주 달라 키워드를 못 씁니다. 대신 **제목에 적힌 날짜**로
  예배를 찾습니다 — `0904` / `260904` / `20260904` / `9월 4일` / `9.4` / `9/4` 를 모두 알아봅니다.
  **제목에 날짜가 있으면 그 날짜만 믿습니다**: 늦게 올려도(예: 9/4 금요예배 글을 9/11 에 올려도)
  9/4 예배에 붙고, 올린 날짜 때문에 다른 주로 끌려가지 않습니다.
  제목에 날짜가 없을 때만 올린 날짜(예배 전날~제출일)로 추정하며, 이때도 제목의 `금요`/`주일` 이
  다른 예배를 가리키면 붙이지 않습니다. 제목 날짜에 맞는 예배가 없으면 **붙이지 않고 비워 둡니다**
  (틀린 주에 붙이는 것보다 낫습니다 — 수동 체크로 처리하세요).
  → 그동안 자동으로 붙은 결과를 되짚어 보려면 **Actions → Admin Tools → `audit-sermons`**
  (출력 전용, 아무것도 고치지 않음). 옛 로직과 결과가 달라진 글 · 붙을 예배가 없는 글 ·
  제목에 날짜가 없어 올린 날짜로 붙인 글을 목록으로 보여 줍니다. `member` 로 한 명만 볼 수 있습니다.
